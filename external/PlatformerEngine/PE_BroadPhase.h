#ifndef _PE_NBROAD_PHASE_H_
#define _PE_NBROAD_PHASE_H_
...
PE_INLINE int PE_BroadPhase_GetPairCount(PE_BroadPhase *bp)
{
...
PE_INLINE PE_ProxyPair *PE_BroadPhase_GetPairs(PE_BroadPhase *bp)
{
...
PE_INLINE bool PE_BroadPhase_Overlap(PE_BroadPhase *bp, PE_Id proxyIdA, PE_Id proxyIdB)
{
    PE_AABB *aabbA = PE_DynamicTree_GetFatAABB(bp->m_tree, proxyIdA);
    PE_AABB *aabbB = PE_DynamicTree_GetFatAABB(bp->m_tree, proxyIdB);
    return PE_AABB_Overlap(aabbA, aabbB);
}

typedef bool PE_QueryCallback(PE_Collider *collider, void *data);

void PE_BroadPhase_Query(PE_BroadPhase *bp, PE_AABB *aabb, PE_QueryCallback *callback, void *data);
