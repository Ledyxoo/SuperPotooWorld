    ...
    for (int x = 0; x < width; ++x)
    {
        for (int y = 0; y < height; ++y)
        {
            ...
            case TILE_STEEP_SLOPE_L:
                vertices[0] = PE_Vec2_Set((float)x, (float)y);
                vertices[1] = PE_Vec2_Set((float)x, (float)y + 1.0f);
                vertices[2] = PE_Vec2_Set((float)x + 1.0f, (float)y);
        ...
        for (int y = 0; y < height; y++)
        {
            ...
            if (StaticMap_GetTileType(map, x, y) == TILE_GROUND)
            {
                ...
                else
                {
                    ...
                    if (StaticMap_IsGround(map, x - 1, y) == true && StaticMap_IsGround(map, x + 1, y) == true && StaticMap_IsGround(map, x + 1, y + 1) == false)
                    {
