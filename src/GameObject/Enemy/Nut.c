...
void Nut_CreateAnimator(Nut *nut, void *scene)
{
    ...
    // Animation "Idle"
     part = RE_Atlas_GetPart(atlas, "NutIdle");
    AssertNew(part);
...
void Nut_VM_FixedUpdate(void *self)
{
   ...
   if (velocity.x == 0 && position.x > playerPos.x)
    {
        velocity = PE_Vec2_Set(-1.0f, 0.0f);
    }
    else if (velocity.x == 0 && position.x < playerPos.x)
    {
        velocity = PE_Vec2_Set(-1.f, 0.0f);
    }
 ...
 /*  else if (dist <= 5.0f && nut->m_state == NUT_IDLE)
    {
       ...
       // LevelScene_SetStartPosition(scene, nut->m_position);
...
void Nut_VM_Update(void *self)
{
    ...
    {
        nut->timedeath += RE_Timer_GetDelta(g_time);

        if (nut->timedeath > 0.9)
        {
            Scene_DisableObject(scene, nut);
        }
    }
