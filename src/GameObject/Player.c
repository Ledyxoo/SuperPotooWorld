...
void Player_OnCollisionStay(PE_Collision *collision)
{
    ...
    if (PE_Collider_CheckCategory(otherCollider, FILTER_ENEMY) && player->invincibility > 0)
    {
        PE_Collision_SetEnabled(collision, false);
    }
...
void Player_CreateAnimator(Player *player, void *scene)
{
    ...
    anim = RE_Animator_CreateAlphaAnim(animator, "Noel", 0.2, 0.8);
    AssertNew(anim);
    RE_Animation_SetCycleCount(anim, 7);
    RE_Animation_SetCycleTime(anim, 0.2f);
    RE_Animation_AddFlags(anim, RE_ANIM_REVERSED);
}
...
void Player_Constructor(void *self, void *scene)
{
    ...
    // Le joueur doit �tre r�initialliser � chaque fois qu'il meurt
...
void Player_Damage(Player *player)
{
    ...
    if (player->invincibility < 0)
    {
        ...
        if (player->m_state != PLAYER_DYING && player->m_heartCount>0)
        {
            RE_Animator_PlayAnimation(player->m_animator, "Noel");
        }
        ...
        if (player->m_heartCount == 0 && player->m_state != PLAYER_DYING)
        {
...
{
    Player* player = Object_Cast(self, Class_Player);
    if (player->m_heartCount < MAX_HEART_COUNT)
    {
...
void Player_VM_FixedUpdate(void *self)
{
    ...
    player->invincibility -= Scene_GetFixedTimeStep(scene);
   ...
   // Mix_Music* bgm = Mix_LoadMUS("bgm.mp3");
    //Mix_Chunk* SoundEffect = Mix_LoadWAV("raindrop.wav");
    // bgm = nullptr;
     //SoundEffect = nullptr;
    ...
    if (onGround)
    {
        if ((player->m_state != PLAYER_IDLE && player->m_state != PLAYER_DYING) && velocity.x == 0.0f)
        {
            player->m_state = PLAYER_IDLE;
            RE_Animator_PlayAnimation(player->m_animator, "Idle");
        }
        else if ((player->m_state != PLAYER_RUNNING && player->m_state != PLAYER_DYING) && fabsf((velocity.x >= 1.0f && player->m_hDirection == 1.0f) || (velocity.x <= -1.0f && player->m_hDirection == -1.0f) && velocity.x != 0.0f))
        {
            player->m_state = PLAYER_RUNNING;
            RE_Animator_PlayAnimation(player->m_animator, "Running");
        }
        else if ((player->m_state != PLAYER_SKIDDING && player->m_state != PLAYER_DYING) && (controls->hAxis == -1.0f && player->m_hDirection == 1.0f) || (controls->hAxis == 1.0f && player->m_hDirection == -1.0f))
        {
    ...
    {
        if (player->m_state != PLAYER_FALLING && player->m_state != PLAYER_DYING)
        {
   ...
   // Mix_FreeChunk(SoundEffect);
    ...
    if (controls->jumpDown == true && player->m_jump == true)
    {   
        player->m_jump = false;
            if (onGround)
            {
                
                PE_Body_SetGravityScale(body, 1.0f/2);
                velocity.y = 15.0f;
            }
    }
    else if (controls->jumpDown == false)
    {
...
void Player_VM_Render(void *self)
{
    ...
    if (player->m_jump && player->m_state != PLAYER_DYING )
    {
        rect.h = 1.5f * scale; // Le sprite fait 1.375 tuile de haut
        rect.w = 1.000f * scale; // Le sprite fait 1 tuile de large
    }
    else if (controls->jumpDown == 0 && position.y < 2.7 && velocity.y < 0 && player->m_state != PLAYER_DYING)
    {
        rect.h = 1.0f * scale; // Le sprite fait 1.375 tuile de haut
        rect.w = 1.000f * scale; // Le sprite fait 1 tuile de large
    }
...
void Player_VM_Update(void *self)
{
    ...
    player->timeinv += RE_Timer_GetDelta(g_time);
